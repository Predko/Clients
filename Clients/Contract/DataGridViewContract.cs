    ...
    public partial class Clients : Form
    {
        ...
        private void SetRowsHeaderValue(int beginIndex)
        {
            ...
            while (index < dataGridViewContract.Rows.Count) // начинаем с заданного индекса
            {
                ...
                if (!row.IsNewRow)
                {
                    row.HeaderCell.Value = String.Format("{0,3}", index + 1);
//                    dataGridViewContract.InvalidateCell(row.HeaderCell);
                }

                index++;
            }

            // пересчитываем размер заголовков строк, если были изменения
            if (index != beginIndex)
            {
        ...
        private void DataGridViewContract_DefaultValuesNeeded(object sender, DataGridViewRowEventArgs e)
        {
            ...
            SetRowsHeaderValue(e.Row.Index);
